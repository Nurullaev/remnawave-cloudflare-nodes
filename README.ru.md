...
- `address` — адрес ноды в Remnawave (`node.address`). Используется, если нода доступна через Tailscale, VPN или иной адрес, отличный от публичного IP. Если не указан, по умолчанию равен `ip`.
